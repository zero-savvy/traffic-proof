...
fn fold_fold_fold(selected_function: String,
    ...
    } else if selected_function == "redact" {
        let input_data: ZKronoInputRedact = serde_json::from_str(&input_file_json_string).expect("Deserialization failed");
        iteration_count = input_data.original.len();
        start_public_input.push(F::<G1>::from(0));
        start_public_input.push(F::<G1>::from(0));
        for i in 0..iteration_count {
            let mut private_input = HashMap::new();
            private_input.insert("block".to_string(), json!(input_data.original[i]));
            private_input.insert("redact".to_string(), json!(input_data.redact[i]));
            private_inputs.push(private_input);
        }
...
fn main() {
    let matches = App::new("VIMz")
        ...
        .arg(
            ...
            .takes_value(true)
            .possible_values(&["crop", "fixedcrop", "grayscale", "resize", "color_transform", "sharpness", "contrast", "blur", "brightness", "hash", "redact"])
        )
        .arg(
    ...
    println!("                                                         ");
    println!(" ██     ██  ██  ███    ███  ████████   Redaction  on JPEG");
    println!(" ██     ██  ██  ████  ████      ███    Supports Compressed");
    println!("  ██   ██   ██  ██ ████ ██     ██      Image    Redaction");
    println!("   ██ ██    ██  ██  ██  ██   ███                         ");
    println!("    ███     ██  ██      ██  ████████████ v2.0.0 ██████████");
    println!(" ________________________________________________________");
